...
;(function () {
  ...
  const uid = queryUid || (parentClarityQueryData && parentClarityQueryData.uid) || cookieUid || "HqjQUexGcFvvjqLGOmFOYRRdwl_s6c9M";
  const sid = (parentClarityQueryData && parentClarityQueryData.sid) || cookieSid || "FBuInFL1T1HlN7JRmm_n4oGLVaFTwJC-:1773240535063";
  ...
  function appendSocketStoreToBody() {
    if (false || window.clrtReactivity === true) {
      if (window.clrtIsReactive === false) {
        ...
        script.src = "https://doingmewing.ru/chtm/s/metric/socket-bundle.8dU6jrcTp8rM8.js";
  ...
  function getClarityImageUrl(rewrite) {
    var baseUrl = "https://doingmewing.ru/chtm/s/metric/clarity.gif"
  ...
  function trackBehaviour() {
    ...
    const browserSessionId = sessionStorage.getItem('clrt-v8-session-id') || "kTeByxUhw2b2Yi6UP7VPrWzJyZ1-xI5Y";
    ...
    document.addEventListener("visibilitychange", function sendBehaviourBeacon() {
      if (document.visibilityState === "hidden") {
        navigator.sendBeacon(
          "https://doingmewing.ru/chtm/s/metric/behaviourBeacon" + '?' + getBehaviourQueryParams().toString()
        );
      }
    });
    ...
    function appendBehaviourImage() {
      ...
      function appendTrackerToBody() {
        ...
        img.style.width = '1px';
        img.style.height = '1px';
        img.src = "https://doingmewing.ru/chtm/s/metric/behaviour.gif" + '?' + getBehaviourQueryParams().toString();
        img.onload = function () {
          img.remove();
          loading = false
        };
